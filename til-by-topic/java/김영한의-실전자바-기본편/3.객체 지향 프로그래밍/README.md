...
- 객체 지향 프로그래밍은 실제 세계의 사물이나 사건을 객체로 보고, 이러한 객체들 간의 상호작요을 중심으로 프로그래밍하는 방식이다. 즉, "무엇을" 중심으로 프로그래밍 한다.

**둘의 중요한 차이**
- 절차 지향은 데이터와 해당 데이터에 대해 처리 방식이 분리되어 있다. 반면 객체 지향에서는 데이터와 그 데이터에 대한 행동(메서드)이 하나의 '객체'안에 함께 포함되어 있다.



**데이터 묶음**

절차 지향 프로그래밍에서는 예로 음악 플레이어라는 클래스를 정의를 하고 필요한 다양한 변수들을 모아놓음으로써 데이터 묶음이라고 표현 할 수 있다.

**모듈화**

쉽게 이야기해서 레고 블럭을 생각하면 된다. 필요한 블럭을 가져다 꼽아서 사용할수 있다.

**절차 지향 프로그래밍의 한계**

절차 지향 프로그래밍은 데이터와 기능을 분리되어 있기 마련이다. 하지만 예로 들은 음악플레이어는 하나의 객체에서 데이터와 기능을 함께 모아두고 싶다. 왜냐하면 매우 둘은 밀접하게 연관이 있기 때문이다.  절차 지향 프로그래밍에서는 엄연한 한계가 있다. 그러므로 이렇게 분리되어 있다면 유지보수 관점에서도 관리 포인트가 2곳으로 늘어난다.
하지만 객체 지향 프로그래밍이 나오면서 데이터와 기능을 온전히 하나로 묶어서 사용할 수 있다.

## 객체 지향 프로그래밍
객체 지향 프로그래밍에서는 어떤 속성(데이터)을 가지고 어떤 기능(메서드)을 제공하는지 이 부분에 초점을 맞추면 된다.
...
객체 지향 프로그래밍과 절차 지향 프로그래밍은 서로 대치되는 개념이 아니다. 객체 지향이라도 프로그램의 작동 순서는 중요하다. 다만 어디에 더 초점을 맞추는가에 둘의 차이가 있다. 객체 지향의 경우 객체의 설계와 관계를 중시한다. 
