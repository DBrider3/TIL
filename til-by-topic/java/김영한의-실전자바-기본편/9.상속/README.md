...
부모클래스의 생성자가 기본 생성자(파라미터가 없는 생성자)인 경우에는 `super()`를 생략할 수 있다.
...
```
정리하자면
결국 `this(...)`를 써서 생성자에서 생성자를 호출해서 생략이 가능하지만 `super()`를 반드시 한번은 호출해야 한다.

## 문제
상속을 통해 중복 제거를 할 수 있는것을 보여준다. 
각 멤버 변수는 `private`로 접근제어자를 선언해주는것을 잊지말자.
생성자뿐만 아니라 메서드에서도 중복이 발생하면 부모클래스에서 메서드를 정의하고 자식클래스에서 `@Override`로 표기해서 오버라이딩을 해준다. 
